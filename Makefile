...
client-typecheck:
	CI=true pnpm --filter audio-client exec tsc --noEmit

client-test:
	CI=true pnpm --filter audio-client run test:audio-filter
...
setup-diarization:
	...
	PYTHONPATH=. uv run --project server --extra diarization python -m server.download_diarization_models
